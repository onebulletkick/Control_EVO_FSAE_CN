...
本目录存放外部 MATLAB 工具：DYC 有无控制报告，以及固定 CarSim Run 下的 PID 圈速优化。脚本只临时设置仿真参数，不保存 `DYC_1_9_test.slx`。
...
完成后命令行打印 `report.html`、`comparison_metrics.csv`、`autocross_where_faster.csv` 和 `plots_presentation` 的路径。`report.html` 是 HTML 总览页，详细数据见 CSV、MAT 和 PNG。
...
Autocross 后处理需要 `comparison_metrics.csv`、`run_results.csv` 和 `signal_data/aligned_dyc_comparison.csv`。Figure-8 后处理需要 `comparison_metrics.csv`、`figure8_segment_metrics.csv` 和 `figure8_segment_delta.csv`。后处理不调用 CarSim/Simulink，只重建 `*_where_faster.csv` 和对应 TXT 结论。
...
- `report.html`：HTML 总览页。
- `effectiveness_analysis.txt`：文字结论。
- `comparison_metrics.csv`：`dyc_off` / `dyc_on` 总体指标对比。
- `run_results.csv`：每次仿真的停止原因、圈速和运行状态。
- `autocross_where_faster.csv` 或 `figure8_where_faster.csv`：快在哪里分析表。
- `autocross_where_faster_analysis.txt` 或 `figure8_where_faster_analysis.txt`：快在哪里文字结论。
...
`control_EVO/optimization/results/` 是本地生成物目录，不提交到 Git。
...
如果圈速变快，但轮胎峰值利用率、最低速度或其他稳定性指标局部变差，结论写为“圈速有效，稳定性证据混合/不足”。详细数据以 CSV/MAT 为准，HTML 只做总览。
...
本目录脚本仅代表固定 CarSim procedure、当前 `simfile.sim` 和本机 CarSim solver/许可证环境下的离线 Simulink/CarSim 仿真表现。不代表 DIL、实时硬件闭环、Live Video、力反馈或实车性能。

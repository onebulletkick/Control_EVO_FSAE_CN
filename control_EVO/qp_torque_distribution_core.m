function [deltaTorque, diagnostics, lastDeltaTorque, tireModel, tireModelReady, tireModelLoadFailed] = ...
    qp_torque_distribution_core(u, lastDeltaTorque, tireModel, tireModelReady, tireModelLoadFailed)
%QP_TORQUE_DISTRIBUTION_CORE QP四轮差动扭矩分配核心算法。
%   从S-Function接口分离出的纯算法，便于测试和复用。
%
%   输入：
%       u                  - 21x1 输入向量（与S-Function接口一致）
%       lastDeltaTorque    - 4x1 上一拍差动扭矩
%       tireModel          - 轮胎查表模型结构体（可为空）
%       tireModelReady     - 模型是否已加载
%       tireModelLoadFailed - 模型加载是否已失败
%
%   输出：
%       deltaTorque        - 4x1 差动扭矩 [L1;R1;L2;R2]
%       diagnostics        - 6x1 诊断量 [yawSlack; achievedYawMoment; drivePowerMargin;
%                            slewConflictCount; targetYawMoment; maxDeltaUtilization]
%       lastDeltaTorque    - 4x1 更新后的差动扭矩（供下一拍使用）
%       tireModel          - 更新后的轮胎查表模型结构体
%       tireModelReady     - 更新后的模型就绪标志
%       tireModelLoadFailed - 更新后的模型加载失败标志
%
...
if ~isfinite(Mz)
    ...
    deltaTorque = lastDeltaTorque(:);
    diagnostics = localBuildDiagnostics(0, achievedYawMoment, ...
...
try
    ...
    deltaTorque = fallbackDelta;
    diagnostics = localBuildDiagnostics(Mz - achievedYawMoment, ...
...
diagnostics = localBuildDiagnostics(X(5), achievedYawMoment, ...
...
function diagnostics = localBuildDiagnostics(yawSlack, achievedYawMoment, ...
...
baseFolder = fileparts(which('qp_torque_distribution_core'));
